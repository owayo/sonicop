...
|---|---:|---:|---:|---:|---:|---:|
| rubocop/rubocop | 1,780 | 5,826 | 12.84 秒 | **2.49 秒** | 46.09 秒 | **8.01 秒** |
| mastodon/mastodon | 3,292 | 15,293 | 18.66 秒 | **3.16 秒** | 35.88 秒 | **7.10 秒** |
| Homebrew/brew | 2,296 | 51,527 | 19.58 秒 | **2.93 秒** | 40.25 秒 | **6.65 秒** |
| rails/rails | 3,562 | 168,615 | 36.24 秒 | **8.79 秒** | 87.41 秒 | **19.64 秒** |
| ruby/ruby | 7,477 | 765,975 | 97.46 秒 | **18.28 秒** | 191.01 秒 | **40.58 秒** |

差は並列で 4.1〜6.7 倍、単一プロセスで 4.5〜6.1 倍と幅があり、1 コーパスでは代表できません。
...
2 回目の実行（キャッシュ命中）は `ruby/ruby` で 1.88 秒、Rails で 0.42 秒です。

測定機は Apple M2（8 コア）、Ruby 4.0.6（YJIT 利用可）、RubyGems 導入の RuboCop 1.89.0。
2026-08-30 に、`rubocop_rubocop` 2693129 / `mastodon_mastodon` b59ddc7 / `Homebrew_brew` b42173b /
`rails_rails` a19f07f / `ruby_ruby` 22e4a75 の各リビジョンに対して測定しました。
1 分平均のロードアベレージは各行の測定時点で 3.3〜10.1 で、その大半は RuboCop 自身の並列
