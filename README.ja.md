...
Mastodon（15,286 件）で、過剰も不足もメタデータ差もありません。対象ファイル一覧は、この 3 つと
Homebrew の 4 つで**件数だけでなくパスまで**一致します（ruby/ruby はパス単位で比較したことが
ありません）。残る差分は `Lint/Syntax` に集中しています。その大半は、本家の LALR パーサが
...
どちらの側も少なく検査してはいない、と言えます。パス単位の一致はより強い主張なので、
上の*適合性*の節で述べています（5 つのうち 4 つ）。
