...
この計測は v26.8.103 時点のもので、コーパスも [CONFORMANCE.md](CONFORMANCE.md) が固定している
commit より数コミット古いものです（ここのファイル数がわずかに少ないのはそのためです）。以後
変わったのは Cop のロジックだけで、解析・ルール振り分け・補正ループには手が入っていません。
秒数はこの commit ではなく、その計測時点のものとして読んでください。
