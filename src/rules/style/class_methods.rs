...
pub(super) fn check(context: &RuleContext<'_>, offenses: &mut Vec<Offense>) {
    for node in context.nodes_of_any(&["class", "module"]) {
        ...
        for definition in super::nodes::children(body) {
            if definition.kind() != "singleton_method" {
                continue;
            }
            let Some(receiver) = definition.child_by_field_name("object") else {
                continue;
            };
            if constant_key(context, receiver).as_deref() != Some(key.as_str()) {
                continue;
            }
            let Some(method) = definition.child_by_field_name("name") else {
                continue;
            };
            // `node.receiver.loc.name` is the last segment of a qualified constant, not the whole
            // of it.
            let reported = match receiver.child_by_field_name("name") {
                Some(last) => last,
                None => receiver,
            };
            let message = format!(
...
fn constant_key(context: &RuleContext<'_>, node: Node<'_>) -> Option<String> {
    match node.kind() {
        ...
        "scope_resolution" => {
            let name = context.source.node_text(node.child_by_field_name("name")?);
