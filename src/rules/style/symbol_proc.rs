...
pub(super) fn check(context: &RuleContext<'_>, offenses: &mut Vec<Offense>) {
    ...
    for block in context.nodes_of_any(&["block", "do_block"]) {
        ...
        if let Some(receiver) = dispatch.receiver {
            if receiver.kind() == "hash" && UNSAFE_HASH_METHODS.contains(&dispatch.method.as_str())
            {
                continue;
            }
            if ARRAY_KINDS.contains(&receiver.kind())
...
fn block_parameter(
    ...
    let parameters = match dispatch.arrow {
        true => dispatch.call.child_by_field_name("parameters"),
        false => block.child_by_field_name("parameters"),
    };
...
fn single_call(context: &RuleContext<'_>, block: Node<'_>, parameter: &str) -> Option<String> {
    ...
    match only.kind() {
        "call" => {
            let receiver = only.child_by_field_name("receiver")?;
            if receiver.kind() != "identifier" || context.source.node_text(receiver) != parameter {
                return None;
            }
            ...
            if only.child_by_field_name("block").is_some() {
                return None;
            }
            if !send_node::arguments(*only).is_empty() {
                return None;
            }
            let method = only.child_by_field_name("method")?;
            Some(context.source.node_text(method).to_owned())
        }
        // `!x` and `-x` are calls upstream, named after the operator.
        "unary" => {
            let operand = only.child_by_field_name("operand")?;
            if operand.kind() != "identifier" || context.source.node_text(operand) != parameter {
                return None;
            }
            let operator = only.child_by_field_name("operator")?;
            Some(
...
fn block_range_with_space(
    ...
    let (open, close) = braces(block);
    let start = match dispatch.call.child_by_field_name("arguments") {
        Some(list)
